...
export default function Prompt() {
  const generateResponse = useCallback(async (input: string) => {
    const completion = await openai.chat.completions.create({
      ...
      messages: [{ role: 'user', content: input + 'Generate a response in JSON format. There should  be 3 fields, studentIncome, househouldIncome, expectedHouseholdContribution.' }],
      max_tokens: 64,
      temperature: 0.2
    });
